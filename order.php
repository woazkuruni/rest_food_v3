    ...
    <div class="container">
        <div class="page-hero-row">
            <div><span class="eyebrow">Secure checkout</span>
                ...
                <p>Delivery, coupon and payment — all totals are rechecked on the server before your order is saved.</p>
            </div>
            <div class="checkout-trust"><span>🔒 Secure forms</span><span>✓ Server-verified total</span><span>↻ Order tracking</span></div>
        </div>
    </div>
</section>
<section class="section checkout-section">
    <div class="container checkout-grid checkout-grid-wide">
            ...
            <form method="post" id="checkout-form" data-checkout-form data-quote-url="<?= e(url('checkout-quote.php')) ?>">
                ...
                <div class="panel checkout-card">
                    <div class="checkout-card-head">
                        ...
                        <div>
                            ...
                            <p class="muted">Choose a saved address or enter a different delivery location.</p>
                        </div>
                    </div>
                    <?php if ($addresses): ?>
                        <div class="field"><label>Saved address</label><select name="address_id">
                                <option value="">Enter manually</option><?php foreach ($addresses as $address): ?><option value="<?= (int)$address['id'] ?>" <?= $address['is_default'] ? 'selected' : '' ?>><?= e($address['label'] . ' — ' . $address['address']) ?></option><?php endforeach; ?>
                            </select></div><br>
                    <?php endif; ?>
                ...
                <div class="panel checkout-card">
                    <div class="checkout-card-head">
                        ...
                        <div>
                            <h2>Coupon</h2>
                            <p class="muted">Apply a coupon to preview the exact payable amount before payment.</p>
                        </div>
                    </div>
                ...
                <div class="panel checkout-card">
                    <div class="checkout-card-head">
                        ...
                        <div>
                            ...
                            <p class="muted">Choose how you want to pay. Mobile payments are verified by the administrator before being marked Paid.</p>
                        </div>
                    </div>
                    ...
                    <div class="payment-box mobile-payment-box" data-mobile-payment hidden>
                        <div class="payment-instruction-head">
                            <div><span class="payment-kicker">Manual mobile payment</span>
                                <h3>Pay <span data-pay-total><?= e(money($displayTotal)) ?></span></h3>
                            </div><span class="status status-warning">Verification required</span>
                        </div>
                        <div class="merchant-number-row">
                            <div><span class="muted">Merchant number</span><strong data-pay-number></strong></div><button class="btn btn-light btn-sm" type="button" data-copy-payment>Copy number</button>
                        </div>
                        <ol class="payment-steps">
                            <li>Send the exact amount shown above to the merchant number.</li>
                            <li>Enter the sender phone and transaction ID below.</li>
                            <li>Submit the order. Admin will verify the transaction before marking it Paid.</li>
                        </ol>
                        <div class="form-grid">
                            <div class="field"><label>Sender phone</label><input class="input" name="payment_phone" inputmode="tel" placeholder="01XXXXXXXXX"></div>
                            <div class="field"><label>Transaction ID</label><input class="input" name="payment_reference" placeholder="e.g. 9AB12CD34E"></div>
                        </div>
                    </div>
        ...
        <aside class="checkout-side">
            <div class="panel checkout-summary" data-order-summary>
                ...
                <div class="checkout-items"><?php foreach ($items as $item): ?><div class="checkout-item"><img class="thumb" src="<?= e(food_image($item['image_name'])) ?>" alt="">
                            <div class="checkout-item-info"><strong><?= e($item['title']) ?></strong><span class="muted"><?= (int)$item['quantity'] ?> × <?= e(money($item['price'])) ?></span></div><strong><?= e(money((float)$item['price'] * (int)$item['quantity'])) ?></strong>
                ...
                <div class="summary-security"><span>🔒</span>
                    <div><strong>Total is protected</strong>
                        <p>Price, stock, coupon and delivery fee are validated again on the server.</p>
                    </div>
                </div>
            </div>
        </aside>
