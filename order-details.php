...
$o = $stmt->fetch();
if (!$o) { flash('error', 'Order not found.'); redirect('my_order.php'); }
...
?>
<section class="page-hero"><div class="container"><div class="page-hero-row"><div><span class="eyebrow">Order tracking</span><h1>Order #<?= 1000 + $id ?></h1><p><?= e(date('M d, Y • h:i A', strtotime($o['order_date']))) ?></p></div><span class="status <?= e(status_class($o['status'])) ?> order-status-large"><?= e($o['status']) ?></span></div></div></section>
<section class="section"><div class="container checkout-grid checkout-grid-wide">
<div class="panel"><div class="order-head"><div><h2>Order items</h2><p class="muted"><?= count($items) ?> item<?= count($items) === 1 ? '' : 's' ?> in this order</p></div></div>
...
<hr class="sep"><div class="summary-row"><span>Subtotal</span><strong><?= e(money($o['subtotal'])) ?></strong></div><?php if ((float)$o['discount'] > 0): ?><div class="summary-row"><span>Discount <?= $o['coupon_code'] ? '(' . e($o['coupon_code']) . ')' : '' ?></span><strong>-<?= e(money($o['discount'])) ?></strong></div><?php endif; ?><div class="summary-row"><span>Delivery</span><strong><?= (float)$o['delivery_fee'] ? e(money($o['delivery_fee'])) : 'FREE' ?></strong></div><div class="summary-row total"><span>Total</span><span><?= e(money($o['total'])) ?></span></div>
</div>
<div class="order-detail-side">
<div class="panel"><h2>Delivery</h2><p><?= e($o['delivery_address']) ?></p><p class="muted"><?= e($o['phone_number']) ?></p></div>
<div class="panel payment-status-card">
<div class="payment-status-head"><div><span class="section-kicker">Payment</span><h2><?= e(payment_label($o['payment_method'])) ?></h2></div><span class="status <?= e(status_class($o['payment_status'])) ?>"><?= e($o['payment_status']) ?></span></div>
