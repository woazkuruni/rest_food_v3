...
if (is_post()) {
    ...
    if (mb_strlen($title) < 2) {
        ...
        try {
            $image = upload_image($_FILES['image'] ?? [], 'Category', null);
            $stmt = db()->prepare('INSERT INTO tbl_category(title,image_name,featured,active,created_at) VALUES(?,?,?,?,NOW())');
            $stmt->execute([$title, $image, $featured, $active]);
...
<div class="panel admin-form">
    <form method="post" enctype="multipart/form-data"><?= csrf_field() ?><div class="field"><label>Title</label><input class="input" name="title" value="<?= e($_POST['title'] ?? '') ?>" required></div><br>
        <div class="field"><label>Image</label><input class="input" type="file" name="image" accept="image/jpeg,image/png,image/webp"></div><br>
        <div class="form-grid">
        ...
        </div><br><button class="btn btn-primary">Add category</button>
    </form>
